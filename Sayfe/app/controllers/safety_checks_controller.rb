class SafetyChecksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @safety_check.save
        format.html { redirect_to @safety_check, notice: 'Safety check was successfully created.' }
        format.json { render :show, status: :created, location: @safety_check }
      else
  ...
  def update
      @safety_check = Safety_check.find_by(user_id: params[:id])
    respond_to do |format|
      if @safety_check.update(res: params[:res])
        format.html { redirect_to @safety_check, notice: 'Safety check was successfully updated.' }
        format.json { render :show, status: :ok, location: @safety_check }
      else
  ...
  private
    ...
    def safety_check_params
      params.permit(:group_id, :friend_id)
    end
end
